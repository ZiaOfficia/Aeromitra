...
export const LeftFixedEnquiryPanel = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("openEnquiryPanel", handler);
    return () => window.removeEventListener("openEnquiryPanel", handler);
  }, []);

  return (
    <>
      {/* Trigger Button — vertical tab on desktop, floating pill on mobile */}
      ...
      <div className="fixed bottom-6 right-4 z-40 lg:hidden">
        <button
          onClick={() => setIsOpen(true)}
          className="bg-primary text-white px-5 py-3 rounded-full shadow-xl hover:bg-opacity-90 transition-all font-bold uppercase tracking-widest text-xs flex items-center gap-2"
        >
          <Mail size={14} />
          Enquire Now
        </button>
