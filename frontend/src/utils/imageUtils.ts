...
export const getClientImageUrl = (imageName: string) => {
  const baseUrl = '/images/clients';
  const cleanImageName = imageName.startsWith('/') ? imageName.substring(1) : imageName;
  return `${baseUrl}/${cleanImageName}`;
};

export const getBannerImageUrl = (imageName: string) => {
  const baseUrl = '/images/bannerimage';
  const cleanImageName = imageName.startsWith('/') ? imageName.substring(1) : imageName;
  return `${baseUrl}/${cleanImageName}`;
};

export const getAccommodationImageUrl = (imageName: string) => {
  const baseUrl = '/images/accommodations';
  const cleanImageName = imageName.startsWith('/') ? imageName.substring(1) : imageName;
  return `${baseUrl}/${cleanImageName}`;
};

export const getLogoUrl = (imageName: string) => {
